...
| :-------------: | :-------------: |
| **shrinkage** | minimize SSE + λ * shrinkage penalty|

* When λ=0, the penalty has no effect; when λ gets higher, coefficient estimates are forced to zero.
...
When α=1, glmnet runs a lasso model. We will also use k-fold cross-validation to generate a set of cross-validation errors for each value of λ. By default, 10-fold cross-validation is used.
